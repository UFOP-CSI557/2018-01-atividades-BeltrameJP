...
public class ESReal {
    ...
    public Individuo executar(int _case) {
   ...
   public void mutacaoGaussiana(Individuo individuo) {
	   Random rnd = new Random();
	   
	   for(int i=0;i<individuo.getCromossomos().size();i++) {
		   if(rnd.nextDouble()<this.pMutacao) {
			   //Multiplicação pelo valor gaussiano;
			   individuo.getCromossomos().set(i, (Double)individuo.getCromossomos().get(i)*rnd.nextGaussian());
